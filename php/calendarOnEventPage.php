<?php
    ...
    $res = mysqli_query($connection, 'SELECT events_id, date_event FROM events WHERE Year(date_event)='.$_POST['year'].' AND place_id='.$_POST['regionId']);
    $my_array = array();
    while($row = mysqli_fetch_array($res)) $my_array[$row['date_event']] = $row['events_id'];

function draw_calendar($month, $year, $action) {
    $calendar = '<center><table cellpadding="0" cellspacing="0" class="b-calendar__tb">';
    ...
    for($list_day = 1; $list_day <= $days_in_month; $list_day++) {
        ...
        {
            $calendar.= '<td align="center" class=" b-calendar__weekend" id='.$day;
            $calendar .= '>';
        }
        else 
        {
            $calendar.= '<td align="center" id='.$day.' class="b-calendar__day';
            $calendar .= '">';
        }
